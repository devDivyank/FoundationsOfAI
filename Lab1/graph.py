...
class Graph:
    ...
    def addVertex(self, key, elevation, terrainType):
        """
        Add a new vertex to the graph.
        :param key: The identifier for the vertex (which is a co-ordinate on the map here)
        :param elevation: The elevation at that co-ordinate vertex
        :param terrainType: The type of terrain at that co-ordinate vertex
        :return: Vertex
        """
        # count this vertex if not already present
        if self.getVertex(key) is None:
            self.numVertices += 1
            vertex = Vertex(key, elevation, terrainType)
    ...
    def getTerrainType(self, key):
        vertex = self.getVertex(key)
        if vertex != None:
            return vertex.terrainType
        else:
            return None
